...
const VIEWER_PAGE = "src/markdown/viewer.html";
const CONTEXT_MENU_ID = "crosshelp:markdown-view";
const VIEWER_ORIGIN = ext.runtime.getURL("");
const VIEWER_URL = ext.runtime.getURL(VIEWER_PAGE);
...
const openViewer = (source?: string | null, destinationTabId?: number) => {
    if (source) {
        ext.storage.local.set({ lastOpenedMarkdown: source });
    }
    const url = toViewerUrl(source ?? undefined);
    if (typeof destinationTabId === "number") {
        ext.tabs.update(destinationTabId, { url });
    } else {
        ext.tabs.create({ url });
    }
};

export const enableMarkdownViewer = (browser: typeof ext) => {
    ...
    browser.contextMenus.onClicked?.addListener?.((info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab) => {
    ...
    browser.webNavigation?.onCommitted?.addListener?.((details: chrome.webNavigation.WebNavigationTransitionCallbackDetails) => {
