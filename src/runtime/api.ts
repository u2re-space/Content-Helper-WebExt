const API_KEY = "";
const API_URL = 'https://api.proxyapi.ru/openai/v1/';

//
export const setApiKey = async () => {
    if (!(!API_URL || (await chrome.storage.local.get('apiUrl'))?.apiUrl)) {
        await chrome.storage.local.set({ apiUrl: API_URL });
    }

    //
    if (!(!API_KEY || (await chrome.storage.local.get('apiKey'))?.apiKey)) {
